function dlmoCollector = runModel
...
% groupDir   = fullfile(programDir, 'Group_1'); % Subject group
...
groupDirs = fullfile(topDir,{'Group_1','Group_2','Group_3','Group_4'});
...
%     lightReading = adjustCS(glasses,lightReading);
    ...
    dlmoCollector.app_dlmo_baseline(iSub) = appDLMO(1);
    [~,idxB] = min(abs(matDT - appDT(1)));
    dlmoCollector.mat_dlmo_baseline(iSub) = matDLMO(idxB);
    
    dlmoCollector.app_dlmo_intervention(iSub) = appDLMO(end);
    [~,idxI] = min(abs(matDT - appDT(end)));
    dlmoCollector.mat_dlmo_intervention(iSub) = matDLMO(idxI);
    
    dlmoCollector.app_time_baseline(iSub) = appDT(1);
    dlmoCollector.mat_time_baseline(iSub) = matDT(idxB);
    dlmoCollector.app_time_intervention(iSub) = appDT(end);
    dlmoCollector.mat_time_intervention(iSub) = matDT(idxI);
...
for iVar = 1:numel(varNames)
    if isnumeric(T.(varNames{iVar})) % If variable is numeric
        ...
        filledT.(varNames{iVar}) = interp1(timeUTC,T.(varNames{iVar}),newTimeUTC,'linear');
    end
end
...
drawnow
% hold on
isOver = overUnder(appDT,appDLMO,matDT,filtDLMO);
% pa = @(x,y,isOver)plotAnnotation(x, y, isOver);
% arrayfun(pa, appDT, appDLMO, isOver)
plotAnnotation(appDT(1), appDLMO(1), true)
% plotAnnotation(appDT(2), appDLMO(2), true)
plotAnnotation(appDT(end), appDLMO(end), true, appDLMO(end)-appDLMO(1))
end



%%
function TF = overUnder(targetX,targetY,referenceX,referenceY)
%OVERUNDER TF = true if target is over reference false if under or equal

% interpolate the referenceY for the targetX
interpY = interp1(referenceX, referenceY, targetX, 'linear');

% compare targetY to interpY
TF = targetY > interpY;
...
annotation('textarrow',Xf,Yf,'String',str)
